                                    ...
                                    <td class="px-6 py-4 border-b align-top">
                                        <div class="flex gap-2">
                                            <form method="POST" action="{{ route('admin.users.restore', $user->id) }}">
                                                @csrf
                                                <button type="submit" class="admin-btn-positive px-3 py-2 rounded-md">Restore</button>
                                            </form>
                                            <a href="{{ route('users.show', $user->id) }}" class="admin-btn-neutral px-3 py-2 rounded-md">View</a>
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-4 text-center text-gray-500">No archived accounts found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
