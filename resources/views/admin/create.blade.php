<x-app-layout>
    ...
    <style>
        :root{
            /* Switch admin colors to a pink palette (visual-only change) */
            --brand-green: #F89EA4; /* primary accent becomes pink for admin UI */
            --brand-maroon: #C33F64; /* stronger pink/magenta for accent text */
            --brand-bg-admin-main: #4D9A51; /* soft pink page background */
            --brand-bg-sidebar: #F89EA4; /* slightly different pale pink for sidebar */
        }
    ...
    <div class="flex min-h-screen font-inter bg-brand-bg-admin-main">
        ...
        <aside class="w-64 bg-brand-bg-sidebar flex flex-col p-6 border-r border-pink-100 shadow-sm">
            <a href="{{ route('dashboard') }}" class="flex items-center space-x-2 mb-8">
                ...
                <span class="text-2xl font-bold text-green-600">EcoSpaces</span>
            </a>
            ...
            <div>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="flex items-center space-x-3 text-brand-maroon hover:text-brand-green p-2 rounded-lg hover:bg-pink-50 transition-colors">
                        <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-6xl mx-auto">
                ...
                <div class="mb-6 flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <h1 id="page-title" class="text-3xl font-bold text-gray-900">Add New EcoSpace</h1>
                    </div>
                                        ...
                                        <td class="px-6 py-4 border-b align-top">
                                            ...
                                            @if(count($imgs))
                                                <div id="carousel-{{ $ecospace->ecospaceID }}" class="flex items-center gap-2" data-images='@json($imgs)' data-index="0">
                                                    <button type="button" data-carousel-id="{{ $ecospace->ecospaceID }}" class="px-2 py-1 bg-white border rounded-md carousel-prev">‹</button>
                                                    <img id="carousel-img-{{ $ecospace->ecospaceID }}" src="{{ $imgs[0] }}" alt="ecospace image" class="w-24 h-16 object-cover rounded-md border" />
                                                    <button type="button" data-carousel-id="{{ $ecospace->ecospaceID }}" class="px-2 py-1 bg-white border rounded-md carousel-next">›</button>
                                        ...
                                        <td class="px-6 py-4 border-b align-top">
                                            <div class="flex flex-col items-stretch gap-3">
                                                <form method="POST" action="{{ route('admin.ecospace.approve', $ecospace->ecospaceID) }}">
                                                    @csrf
                                                                                <button type="submit" class="w-full flex items-center justify-center gap-2 bg-pink-600 text-white px-4 py-2 rounded-lg font-semibold shadow hover:bg-pink-700 transition focus:outline-none">Approve</button>
                                                </form>
                                                ...
                                                <td class="px-6 py-4 border-b align-top">
                                                    ...
                                                    @if(count($imgs))
                                                        <div id="e-carousel-{{ $event->eventID }}" class="flex items-center gap-2" data-images='@json($imgs)' data-index="0">
                                                            <button type="button" data-event-id="{{ $event->eventID }}" class="px-2 py-1 bg-white border rounded-md e-carousel-prev">‹</button>
                                                            <img id="e-carousel-img-{{ $event->eventID }}" src="{{ $imgs[0] }}" alt="event image" class="w-24 h-16 object-cover rounded-md border" />
                                                            <button type="button" data-event-id="{{ $event->eventID }}" class="px-2 py-1 bg-white border rounded-md e-carousel-next">›</button>
                ...
                <script>
                    ...
                    function showSection(name, pushState = true) {
                        ...
                        if (name === 'events') {
                            ...
                            btnEco.classList.remove('bg-pink-50');
                            btnEvt.classList.add('bg-pink-50');
                        } else {
                            evt.classList.add('hidden');
                            eco.classList.remove('hidden');
                            btnEvt.classList.remove('bg-pink-50');
                            btnEco.classList.add('bg-pink-50');
                        }
                        // Update header title to reflect active section
                        if (pageTitle) {
                            if (name === 'events') {
                                pageTitle.textContent = 'Manage Events';
                            } else if (name === 'users') {
                                pageTitle.textContent = 'Manage Users';
                            } else {
                                pageTitle.textContent = 'Add New EcoSpace';
                            }
                        }
