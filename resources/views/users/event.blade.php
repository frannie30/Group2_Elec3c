            ...
            <div class="bg-white/90 shadow-2xl sm:rounded-2xl p-6 border border-pink-200">
                ...
                <div class="mb-6">
                    @if(count($imgs))
                        <div id="detail-carousel" class="relative">
                            <img id="detail-img" src="{{ $imgs[0] }}" alt="{{ $event->eventName }}" class="w-full h-72 object-cover rounded-md" />
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="md:col-span-2">
                        <h3 class="text-2xl font-bold text-pink-800">{{ $event->eventName }}</h3>
                        <p class="text-sm text-pink-600 mt-2">{{ $event->eventDesc ?? 'No description provided.' }}</p>
                        ...
                        <div class="mt-4 text-sm text-pink-700">
                            ...
                            <div><strong>Type:</strong> {{ $event->eventType->eventTypeName ?? 'N/A' }}</div>
                        </div>
                    </div>
                    <div class="md:col-span-1 bg-pink-50 p-4 rounded-md">
                        <div class="mb-3"><strong class="text-pink-700">Price Tier</strong>
                            <div class="text-pink-600">{{ $event->priceTier->pricetier ?? 'N/A' }}</div>
                        </div>
                        <div class="mb-3"><strong class="text-pink-700">Status</strong>
                            <div class="text-pink-600">{{ $event->status->status ?? 'N/A' }}</div>
                        </div>
                        <div class="mb-3"><strong class="text-pink-700">Organizer</strong>
                            <div class="text-pink-600">{{ $event->user->name ?? 'Unknown' }}</div>
                        </div>
                        ...
                        <div class="mt-4">
                                ...
                                <button type="button" onclick="alert('Add images is not implemented yet.')" class="w-full block text-center bg-pink-600 text-white px-4 py-2 rounded-md">Add Images</button>
                            </div>

                            <div class="mt-2">
                                {{-- Edit button visible only to the organizer; if this is the organizer's first event show a static (disabled) button --}}
                                @auth
                                    @if(auth()->id() == $event->userID)
                                        @php
                                            $firstEventId = \App\Models\Event::where('userID', $event->userID)
                                                ->orderBy('dateCreated', 'asc')
                                                ->value('eventID');
                                            $isOrganizerFirst = $firstEventId && $firstEventId == $event->eventID;
                                        @endphp
                                        ...
                                        @else
                                            <a href="{{ route('admin.event.edit', $event->eventID) }}" class="block text-center bg-yellow-500 text-white px-4 py-2 rounded-md">Edit</a>
                                        @endif
                                    @endif
                                @endauth

                                <a href="{{ url()->previous() }}" class="block text-center bg-pink-600 text-white px-4 py-2 rounded-md mt-2">Back</a>
                            </div>
                        </div>
                    </div>
                </div>
